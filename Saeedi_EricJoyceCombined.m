%This is Saeedi Team's super swell Building Hypothesis Code. Welcome.
%For someone looking for quick info, the output is explained in comment at
%bottom of code. The first input is automatically snagged from cellarrayoutput,
%"roofhypo" although the input in line 14 will need to be changed based on
%the input from the segmentation step. 
tic
...
avgthresh = 1.2; %The value of the avg blue/green pixel threshold. Any hypotheses with a value above this is eliminated.
medthresh = 1.2; %The median value equivalent of the above.
stdthresh = 1.2; %Standard deviation equivalent of above. Least likely to use in a meaningful way but interesting try nonetheless. 
input = load('sampleseg3lines.mat'); %%Input files
...
%Uncomment these to print the hypotheses on the graph.
...
%     hold on
    ...
    for i = int16(ceil(minx)):int16(ceil(maxx)); %%Rename if changing metric
        for j = int16(ceil(miny)):int16(ceil(maxy)); %%Rename if changing metric 
    ...
    if val == 0; 
        roofhypo{khypo} = [NaN;NaN;NaN;NaN;NaN]; %Replace the entry in roofhypo with NaN.
    end
...
%         line([v4x2cord,v1x2cord],[v4y2cord,v1y2cord], 'color', 'green', 'LineWidth', 2)
    ...
    %than the input angle value (mintheta parameter).
    ...
    for k = 1:length(chosenlinescell);
        ...
        %Let's remove the vectors that are more than mintheta degrees different
    ...
    %Now we have a cell array that contains the matrices of every [x1,x2;y1,y2]
    %line that is within each tube for Tube 3 in chosenlinescell. 
    ...
    %Now we have a cell array that contains the matrices of every [x1,x2;y1,y2]
    %line that is within each tube for Tube 4 in chosenlinescell. 
...
%outputlinesandscores is an ix2 cell such that every row first column contains a 2x8
%matrix, representing a hypothesis of four lines, and every row second
%column contains the score assigned to that particular hypothesis. There
%are as many rows as there are potential hypotheses that have made it past
%the filtering procedure. finaloutput is a 1xn cell where n is the total
%number of hypotheses iterated over. Each column contians
%outputlinesandscores for the given hypothesis. 

%Additional info on outputlinesandscores:
